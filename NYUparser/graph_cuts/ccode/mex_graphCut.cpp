	...
	for (int i = 0; i < nrows; i++){
		for (int j = 0; j < ncols; j++){
			...
			if( i < nrows-1){
				...
				assert(term01 >= 0);
                g -> add_edge(i+j*nrows,(i+1)+j*nrows,term01,term01);
			}
			//horizontal edges (i,j) <-> (i,j+1)
			if( j<ncols-1){
				term01 = lambda1+lambda2*getEdgeTerm(i+j*nrows, i+(j+1)*nrows, nodeNum, ImagePtr, EdgeBeta);
				assert(term01 >= 0);
                g -> add_edge(i+j*nrows,i+(j+1)*nrows,term01,term01);
				
			}	
			if(NeighSystem){
				//diagonal edges (i,j) <-> (i+1,j+1) 
				if( i<nrows-1 && j<ncols-1 ){
					term01 = lambda1+lambda2*getEdgeTerm(i+j*nrows,(i+1)+(j+1)*nrows, nodeNum, ImagePtr, EdgeBeta);
					assert(term01 >= 0);
					g -> add_edge(i+j*nrows,(i+1)+(j+1)*nrows,term01/sqrt(2.0),term01/sqrt(2.0));
				}
				//diagonal edges (i,j) <-> (i-1,j+1)
				if ( i > 0 && j<ncols-1){
					term01 = lambda1+lambda2*getEdgeTerm(i+j*nrows,(i-1)+(j+1)*nrows, nodeNum, ImagePtr, EdgeBeta);
