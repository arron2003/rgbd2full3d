function data = findLayout(data, opt)
  ...
  addpath([fileparts( mfilename('fullpath') ) '/../util/liblinear_dense']);
  ...
  for s=layoutLabel
    ...
    for h = v
      ...
      m2 = ((1-abs((norm_3d(m,:)*result(cnt).plane(1:3)')))/opt.tolPara/2).^2;
      result(cnt).mask(m) = m1+m2;
      
      % find connected component and add holes
      holemap = result(cnt).mask>4;
  ...
  for i=1:numel(result)
    result(i).vertices = result(i).vertices*data.R';
