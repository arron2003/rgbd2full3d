...
function [imgPlanes, imgNormals, imgConfs] = ...
    compute_local_planes(X, Y, Z, params)
  ...
  blockWidths = params.planes.blockWidths;
  ...
  for k = ind_all(:)'        
    ...
    valid = abs(Z(ind2) - Z(k)) < Z(k) * params.planes.relDepthThresh;
    u2 = u2(valid);
    v2 = v2(valid);
    ind2 = v2 + (u2-1)*H;
