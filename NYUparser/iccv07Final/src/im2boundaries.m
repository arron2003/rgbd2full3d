function bndinfo = im2boundaries(im, pbim, gconf, dtBnd, dtBnd_fast, dtCont, ...
...
result = mergeStageMin(X, bndinfo, dtBnd(s), dtBnd_fast(s), 2, thresh(s));
...
s = 2;
X = getFeatures(bndinfo, im, pbim, gconf);
result = mergeStageBp(X, bndinfo, dtBnd(s), dtBnd_fast(s), dtCont(s), thresh(s));
bndinfo = updateBoundaryInfo(bndinfo, result, im); 
...
[bndinfo, lab, plab_e, plab_g] = mergeStageFinalBpGeometry(...
